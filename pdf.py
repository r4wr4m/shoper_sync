...
class SimpleInvoice(BaseInvoice):
    ...
    def _drawProvider(self, TOP, LEFT):
        self._drawAddress(TOP, LEFT, 88, 36, _(u'Sprzedawca'), self.invoice.provider)

    def _drawPayment(self, TOP, LEFT):
        self.pdf.setFont('DejaVu-Bold', 8)
        self.pdf.drawString(LEFT * mm, (TOP + 2) * mm, _(u'Informacje dotyczące płatności'))

        text = self.pdf.beginText((LEFT) * mm, (TOP - 2) * mm)
        lines = [
            self.invoice.provider.bank_name,
            '%s: %s' % (_(u'Nr rachunku'), self.invoice.provider.bank_account_str()),
        ]
    ...
    def _drawItemsHeader(self,  TOP,  LEFT):
        ...
        self.pdf.drawString((LEFT + 1) * mm, (TOP - 2) * mm, _(u'Zamówione towary'))

        self.pdf.drawString((LEFT + 1) * mm, (TOP - 9) * mm, _(u'Nazwa'))
        items_are_with_tax = self.invoice.use_tax
        if items_are_with_tax:
            i = 9
            self.pdf.drawString((LEFT + 73) * mm, (TOP - i) * mm, _(u'Ilość'))
            self.pdf.drawString(
                (LEFT + 88) * mm,
                (TOP - i) * mm,
                _(u'Cena'),
    ...
    def _drawItems(self, TOP, LEFT):  # noqa
        ...
        if not items_are_with_tax:
            ...
            self.pdf.setFont('DejaVu-Bold', 7)
            self.pdf.drawString((LEFT + 100) * mm, (TOP - i - 10) * mm, '(%s: %s)' % (_(u'Słownie'),self.invoice.specific_symbol))
        else:
