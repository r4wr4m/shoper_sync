import sys,requests,re,json,time,pickle,time
...
#Scraping data
    ...
    for item in j['items']: #for every 25 responses
        ...
        for product in item['body']['list']: #for products in single response
            ...
            delivery_id_in_other_shop='PLACEHOLDER'
            availability_id_in_other_shop='PLACEHOLDER'
            products.append({
                'id':id,
                'name':name,
                'stock':stock,
                ...
                'delivery_id_in_other_shop':delivery_id_in_other_shop,
                'availability_id_in_other_shop':availability_id_in_other_shop,
                'is_set':is_set,
                'children':children
                })
    return products
...
def save_products(products,filename):
    pickle.dump(products, open(filename, 'wb'))
    print(Fore.GREEN+'[+] Products saved to file: ' + filename)

def load_products(filename):
    products = pickle.load(open(filename, 'rb'))
...
def compare_dictionary_names(dictionary1,dictionary2,page1,page2,dictionary_content,print_diff=True): #compare product names in dictionaries
    ...
    if len(diff1)>0 or len(diff2)>0:
        ...
        #if print_diff:
            #print('[+] All {} are the same'.format(dictionary_content))
        return True
...
def compare_products(products1,products2,name_dict1,name_dict2,page1,page2,availabilities1,availabilities2,deliveries1,deliveries2,attributes,verbose,print_details):
    ...
    if verbose:
        ...
        if same:
            print(Fore.GREEN+'THE SAME DATABASES:\t\t'+str(same))
        else:
            print(Fore.RED+'THE SAME DATABASES:\t\t'+str(same))

    return all(tests) #if all tests are True 
...
def test(page):
    ...
    #print_pretty_json(requests.get('https://'+page+'/webapi/rest/products?limit=1&page=1&filters={"product_id":7169,"translations.pl_PL.active":"1"}',headers=headers,proxies=proxies,verify=verify).text)
    #print_pretty_json(requests.get('https://'+page+'/webapi/rest/availabilities',headers=headers,proxies=proxies,verify=verify).text)
    #print_pretty_json(requests.get('https://'+page+'/webapi/rest/deliveries',headers=headers,proxies=proxies,verify=verify).text)
...
if online:
    ...
    products=[
                api_get_products(pages[0][0],pages[0][3],availabilities[0],deliveries[0],False),
                api_get_products(pages[1][0],pages[1][3],availabilities[1],deliveries[1],False)  
             ]
...
changes=0
#changes += copy_attribute(pages[0],pages[1],name_dict1,name_dict2,'active',False)
#changes += copy_attribute(pages[0],pages[1],name_dict1,name_dict2,'stock',False)
#changes += copy_attribute(pages[0],pages[1],name_dict1,name_dict2,'price',False)
#changes += copy_attribute(pages[0],pages[1],name_dict1,name_dict2,'availability_name',False,availabilities[1])
#changes += copy_attribute(pages[0],pages[1],name_dict1,name_dict2,'delivery_name',True,deliveries[1])
print(changes)
compare_products(products[0],products[1],name_dict1,name_dict2,pages[0][0],pages[1][0],availabilities[0],availabilities[1],deliveries[0],deliveries[1],['active','stock','price','availability_name','delivery_name'],True,False)
